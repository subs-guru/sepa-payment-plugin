...
class SEPANotificationsMailer extends PaymentsNotificationsMailer
{
    ...
    public function exported(PaymentMean $paymentMean, Payment $payment)
    {
        ...
        if ($payment->getAmount() > 0) {
            $this
                ->setTemplate('SubsGuru/SEPA.' . $language . '/payments/exported-payment')
                ->setLayout('SubsGuru/SEPA.' . $language . '/default')
                ->setSubject(__d('SubsGuru/SEPA', 'Levy notification for your subscription to {0}', Configure::read(CONFIG_KEY . '.service.name')));
        }
    }
